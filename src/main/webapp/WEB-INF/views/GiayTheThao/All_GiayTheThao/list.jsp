                ...
                <div class="scrollable-list" style="width: 100%; max-height: 300px; overflow-y: auto; overflow-x: hidden">
                    <table class="data-table" style="margin-top: 30px;width: 100%;border-radius: 50px 50px 50px">
                        ...
                        <c:forEach var="giayTheThao" items="${listPageFind}" varStatus="i">
                            <tr class="product-item hide-row" data-tenGiayTheThao="${giayTheThao.tenGiayTheThao}" data-giaBan="${giayTheThao.giaBan}" data-thuongHieu="${giayTheThao.thuongHieu.tenThuongHieu}" data-congDung="${giayTheThao.congDung}" data-trangThai="${giayTheThao.trangThai}">
								...
								<td style="padding-top: 3px; text-align: center; color: black">
								    <c:if test="${not empty giayTheThao.image and not empty giayTheThao.image.get(0).link}">
								        <img style="width: 100px; margin-bottom: 10px" src="/upload/${giayTheThao.image.get(0).link}" alt="Ảnh sản phẩm">
								    </c:if>
								    <c:if test="${empty giayTheThao.image or empty giayTheThao.image.get(0).link}">
								        <span>NULL</span>
								    </c:if>
								</td>
...
<script>
    ...
    function filterProducts() {
		...
		document.getElementById('trangThai').addEventListener('change', function () {
		    filterProducts(); // Gọi hàm lọc sản phẩm ngay khi thay đổi
		});
