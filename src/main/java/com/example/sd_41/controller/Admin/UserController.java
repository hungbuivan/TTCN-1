...
public class UserController {
    ...
    @GetMapping("/loc")
    public String loc(@RequestParam(value = "trangThai", required = false) Integer trangThai, Model model) {
        List<User> searchResults;

        // In giá trị của trangThai để kiểm tra
        System.out.println("TrangThai: " + trangThai);

        // Nếu không có giá trị trangThai, lấy tất cả người dùng
        if (trangThai == null) {
            searchResults = userService.getAll();
        } else {
            // Lọc theo trạng thái, nếu trangThai == 0 thì lấy người dùng đang hoạt động
            searchResults = userService.searchByTrangThai(trangThai);
        }
